...
csv_path = './src/severity/output_pca.csv'
...
for fold_index, (train_idx, val_idx) in enumerate(kf.split(X, y)):
    ...
    if val_accuracy > best_accuracy:
        ...
        joblib.dump(rf_model, "./src/severity/models/2_2_fourier_pca_rf_severity.pkl")

# Summary of cross-validation
print("\nCross-Validation Results:")
print(f"Mean Validation Accuracy: {np.mean(fold_scores):.4f}")
print(f"Best Validation Accuracy: {best_accuracy:.4f}")
print(f"Best model saved as: {"./src/severity/models/2_2_fourier_pca_rf_severity.pkl"}")